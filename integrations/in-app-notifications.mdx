...
`socket.io-client` is an optional peer dependency. The SDK finds it via `window.io`, a dynamic import, or an explicit `ioClient` option.
...
<ParamField path="options.signMessage" type="(message: string) => Promise<string>">
  Custom signer. Defaults to `personal_sign` via `window.ethereum`.
</ParamField>
...
<ParamField path="options.onNotification" type="(n) => boolean | void">
  Called per notification. Return `false` to suppress the built-in toast.
</ParamField>
